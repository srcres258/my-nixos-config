...
, ...
  ...
  # ── VS Code user settings (declarative, used as baseline) ──
  vscodeUserSettings = {
    ...
    # ── Prevent extensions from writing settings.json on every startup ──
    # Scala Metals – exact patterns from configureSettingsDefaults()
  ...
  vscodeSettingsJson = builtins.toJSON vscodeUserSettings;
  vscodeSettingsNixStore = pkgs.writeText "vscode-settings-nix.json" vscodeSettingsJson;

  # Writable target outside the Nix store
  vscodeWritablePath = "${config.home.homeDirectory}/.config/Code/User/vscode-settings-writable.json";
in {
  ...
  # ── Activation: merge Nix baseline with user changes into a writable settings file ──
  home.activation.initVscodeWritableSettings = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    TARGET="${vscodeWritablePath}"
    NIX_SOURCE="${vscodeSettingsNixStore}"

    if [ ! -f "$TARGET" ]; then
      # First run: copy Nix baseline verbatim
      cp "$NIX_SOURCE" "$TARGET"
    else
      # Subsequent runs: merge — Nix-managed keys override, user-only keys preserved
      ${pkgs.jq}/bin/jq -s '.[0] * .[1]' "$TARGET" "$NIX_SOURCE" > "$TARGET.tmp" \
        && mv "$TARGET.tmp" "$TARGET"
    fi
    chmod 644 "$TARGET"
  '';

  # ── Override settings.json symlink → writable file (overrides programs.vscode managed file) ──
  xdg.configFile."Code/User/settings.json".source =
    lib.mkForce (config.lib.file.mkOutOfStoreSymlink vscodeWritablePath);
}
