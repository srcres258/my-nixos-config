...
let
  ...
  singBoxSyncConfig = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # urltest: auto (only when proxy nodes exist)
      if proxy_tags:
          outbounds.append({
              "type": "urltest",
              "tag": "auto",
      ...
      # selector: proxy — include "auto" only when urltest was created
      selector_outbounds = list(dict.fromkeys(
          proxy_tags + (["auto", "direct"] if proxy_tags else ["direct"])
      ))
      outbounds.append({
          "type": "selector",
          "tag": "proxy",
...
in
{
  ...
  systemd.services.singbox-sync = {
    ...
    serviceConfig = {
      ...
      # Restart sing-box after config regeneration so it picks up new nodes.
